...
def Getcam(data): #WHITE
    ...
    # PERSPECTIVE POINT
    pts1 = np.float32([[int(cv_image.shape[1]*0.13),int(cv_image.shape[0]*0.6)],[int(cv_image.shape[1]*0.74),int(cv_image.shape[0]*0.6)],[0,int(cv_image.shape[0]*0.67)],[int(cv_image.shape[1]*0.90),int(cv_image.shape[0]*0.70)]]) #tl,tr,bl,br
    ...
    cv2.circle(cv_image,(0,int(cv_image.shape[0]*0.67)),1,(0,0,255),2)
    cv2.circle(cv_image,(int(cv_image.shape[1]*0.90),int(cv_image.shape[0]*0.70)),1,(0,0,255),2)
    cv2.circle(cv_image,(int(cv_image.shape[1]*0.13),int(cv_image.shape[0]*0.6)),1,(0,0,255),2)
    cv2.circle(cv_image,(int(cv_image.shape[1]*0.74),int(cv_image.shape[0]*0.6)),1,(0,0,255),2)
    ...
    Servo_morter = (Servo_morter0+Servo_morter1)/2 # mean value
    # print(Servo_morter)
    ###################################################
    ...
    x2=420
    kernel_traff = np.ones((5,5),np.float32)/25
    crop_frame_traff = cv_image[12:57 , 152:253]
    ...
    if (np.sum(red_traffic)/255) > 500 :
      ...
      run = 2
      result_traff = hsv_traff
    #cv2.imshow('normal', cv_image)
    cv2.imshow('light', result_traff)
